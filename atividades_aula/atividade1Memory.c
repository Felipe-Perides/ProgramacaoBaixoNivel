...
int main(){
    ...
    printf("Escreva as dimensões da matriz desejada(Altura -> 3 digitos): \n");
    char altura[4];
    fgets(altura, sizeof(altura), stdin);
    int alt = atoi(altura);

    printf("Escreva as dimensões da matriz desejada(Largura -> 3 digitos): \n");
    char largura[4];
    ...
    printf("\nMatriz com distâncias Euclidianas:\n");
    for (int i = 0; i < alt; i++) {
        for (int j = 0; j < larg; j++) {
            printf("%c ", matriz[i][j]);
        }
